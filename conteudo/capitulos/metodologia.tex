...
tráfego de veículos no trânsito.
  ...
  Utilizaremos um cenário de simulação do tráfego de carros e ônibus na cidade
de São Paulo criado e disponibilizado por \citet{santana2018courb}, com cerca
de 585 mil veículos e mais de 31 milhões de pontos. Para construir o arquivo
\emph{trips.xml} com a lista de viagens, eles utilizaram uma matriz de
...
espaciais faremos agrupamentos baseados na escala da visualização, que está
relacionada ao tamanho da área do mapa mostrada na imagem. Este mecanismo é
comumente visto em visualizações interativas que permitem aos usuários mudar as
configurações de zoom no mapa. A partir daí, propomos o conceito de
sub-trajetória, que são as partes de uma trajetória dentro da sub área do mapa
após um aumento do zoom. Desta forma detectamos os pontos dentro do escopo da
sub área do mapa e obtemos um subconjunto das trajetórias originais. Os
atributos de direção, origens e destinos do subconjunto se tornam relativos em
relação ao novo escopo. Em seguida é possível reaplicar o \emph{bundling} no
subconjunto. Quanto menor a escala, mais detalhes sobre o fluxo do tráfego são
obtidos, sendo possível observar bairros, quadras ou até mesmo ruas. A Figura
\ref{fig:multi-scale} ilustra os diferentes níveis de detalhe atingidos em
escalas diferentes na visualização de um mapa.
...
para visualização dos fluxos de OD no tráfego. Avaliaremos então as seguintes questões:
  ...
  Do ponto de vista quantitativo pretendemos avaliar o desempenho da solução
