...
coleta. Muitas vezes é necessária uma etapa de pré-processamento para remover
...
\subsection{Quanto à Exibição das Propriedades Temporais}
...
de visualização é fácil de representar, porém limitada na quantidade de
...
eficiência na representação das propriedades espaciais, \citep{Chen2015}.
...
informação em que o tempo é uma variável de destaque. \citet{Cairo2016} mostra uma série
...
\subsection{Quanto à Exibição das Propriedades Espaciais}
...
Esse tipo de visualização mostra a posição instantânea dos objetos em um certo momento
...
No entanto, à medida que o número de linhas cresce, aumenta também a
oclusão, o que acaba afetando a estética da visualização e a obtenção de
...
espessura, bem como a direção é representada com um pequeno corte no desenho que indica o destino
dos fluxos, como é ilustrado na Figura~\ref{fig:interchange-circo}a. Uma boa visualização oferece
...
que pode ser necessário para a identificação das correlações e padrões.
Uma outra técnica conhecida como \emph{bundling} busca resolver tais
